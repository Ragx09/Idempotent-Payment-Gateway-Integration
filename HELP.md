# Idempotent Payment Gateway Integration

This project is a **resilient payment microservice** for **idempotent payment gateway integration** in a broader microservices architecture.

It highlights:

- **Webhooks** with **HMAC signature validation** and **retry handling** (persisted event state + scheduled retries).
- **Distributed locking** (Redis) together with **database pessimistic locking** so concurrent traffic (validated at **4K+** requests via k6) does not corrupt balances or double-apply the same logical payment.

For how to run it, see `README.md` (Docker Compose for Postgres and Redis, then `mvnw spring-boot:run`).

---

# Read Me First (Spring Initializr)
